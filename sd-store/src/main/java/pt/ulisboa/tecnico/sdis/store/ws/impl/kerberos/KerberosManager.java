...
/**
 * Used to manage Kerberos protocol, in servers side, verify the authenticity of
 * the request/client.
 * 
 * @author André
 */
public class KerberosManager {
	...
	public KerberosManager(String serviceId) throws Exception {
		...
		if (SystemUtils.IS_OS_WINDOWS)
			...
			currentKeysFile = System.getProperty("user.dir")
					+ KEYS_FILE_LINUX_MAC;
		loadServerKey();
	}
	...
	/**
	 * Validate a client request, "Kerberos cerebrus", and returns kerberos
	 * authenticator for response.
	 * 
	 * @param base64Ticket
	 * @param base64Auth
	...
	public byte[] processRequest(byte[] byteTicket, byte[] auth,
		...
		try {
			...
			authentication = KerberosClientAuthentication
					.deserialize(auth, kcs);
			...
			addLastRequest(authentication.getClient(),
					authentication.getRequestTime());
